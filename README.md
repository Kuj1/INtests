...
- Для генерации отчета в allure-style: 
  ...
  ```
- В проекте, акивно используется ```seelenium``` и ```chromedriver```. Важно вв проекте используется обычный ```seelenium```, в связи с этим нужен браузер Google Chrome. (ВАЖНО!!! Слеедить за актуальностью драйвера)
